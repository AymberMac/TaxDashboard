...
statesLayer.queryFeatures(query)
  ...
  loadStates()
///query an additional service to add more data to the mapping dashboard
...
//Create a feature layer from additional service URL
  var employmentStatUrl = "https://services.arcgis.com/P3ePLMYs2RVChkJx/ArcGIS/rest/services/ACS_10_14_Employment_Status_Boundaries/FeatureServer/0"
  const employmentStatLayer = new FeatureLayer({
    url: employmentStatUrl,
    title: "Unemployment Rate",
  });

//Make function to execture query for employmentStat column
var loadAdditionalData =function(){
  ////Create a new query object and give it the correct properties for employmentStats
  let additionalQuery = employmentStatLayer.createQuery();
  additionalQuery.returnGeometry = false;
  additionalQuery.outFields = "*"
  ////execute query on employmentStat layer
  employmentStatLayer.queryFeatures(additionalQuery).then(function(r){
console.log(r)
r.features.forEach(function(feature){
  ...
  if(feature.attributes['STUSPS'] != "DC"){
      ...
      var employment = feature.attributes['B23025_calc_pctUnempE']
    
      stateObj[stateName]['employmentStat'] = employment
  }
